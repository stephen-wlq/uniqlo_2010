...
if(isset($_POST['submit'])){//前端点击了submit提交按钮，后端开始接收值。
    ...
    $conn->query("insert registry values(null,'$user','$pass','$email','$tel',NOW())");//将数据传递给数据库。
    //一旦数据提交成功，回到前端的登录页面
    header('location:http://10.31.161.128/dashboard/js%e9%a1%b9%e7%9b%ae%e8%80%83%e8%af%95/uniqlo_2010/src/login.html');
}
